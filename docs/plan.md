...
- 当前里程碑: **设置界面 DTK 化重设计（公子指派，已提交 90e3ea0）**——设计稿 docs/design/settings-redesign.html（可交互 HTML，含深浅两主题）→ 落地为 src/settingsstyle.{h,cpp}（主题感知 QSS + QPainter 自绘导航图标/胶囊开关/主题色板，零 DTK 头依赖、零 SVG 插件依赖，DTK/generic/macOS/Windows 同一代码路径，PaletteChange 排队重刷）+ settingsdialog 重构（分组卡片/语义色横幅/server 状态点与版本徽标/主题色板卡）。随设计稿新增功能位：恢复默认、设置搜索、勿扰时段（appcore onAgentAttention 过滤，支持跨午夜区间）、打开日志文件夹（main.cpp 文件日志 2×1MB 轮转，Platform::logFilePath）、GitHub/反馈入口（updater projectPageUrl）、上次检查时间（updater 写 updater/lastCheckAt/<name>，为 M9 节流铺键）；DDialog OK 按钮改 ButtonRecommend 蓝、generic 壳 OK 按钮同步 primary 样式。lupdate 刷新 ts（顺手完成 M10 行号漂移项）+ zh_CN 补译 61 条。Xvfb 双壳验收通过（generic 全四页 + DTK 终端页/搜索/恢复默认截图核对，零 QCssParser/零 SIGSEGV/零警告构建）
- 最近提交: 90e3ea0 feat: redesign the settings dialog in DTK style
- 断点/下一步: ①公子真机验收项：DTK 壳深色主题切换（Xvfb 无法验深色）、中文翻译实机显示（staging 无 /usr/share 翻译路径，Xvfb 下 DTK 壳显示英文属预期）、勿扰时段真实通知流；②之后回到 M9（更新健壮化，lastCheckAt 键已铺）
