...
- 当前里程碑: **M8 发布进行中**：M7（eddc266）+ 状态带删除（47fb4fb）+ 命名迭代（d65a2f5）+ v0.3.0（64b0d29）+ README（9f1d609）+ 三枚 macOS CI 修复（f1351e5/1e943ad/c10c489）已推送，tag v0.3.0 已重推至 c10c489，CI run 35590128600 进行中（linux 双平台 ✅ / macos-aarch64 ✅ / macos-x86_64 排队——Intel runner 稀缺）。release job 会自动建 release 并上传 8 件资产
- 最近提交: c10c489 fix(ci): link iconv into terminalwidget on macOS；1e943ad fix(ci): uchardet include root；f1351e5 fix(ci): __uid_t→uid_t + PKG_CONFIG_PATH；9f1d609 docs: README；64b0d29 chore: bump 0.3.0
- 断点/下一步: ①等 run 35590128600 全绿 → `gh release view v0.3.0` 验 8 件资产（mudi-{linux-x86_64,linux-aarch64,macos-aarch64,macos-x86_64} 各裸二进制+tar.gz）→ `gh api repos/re2zero/mudi/releases/latest` 验自更新 API（**依赖仓库改名 re2zero/mudi，旧名 re2zero/deepin-herdr 会 302 重定向**，updater 的 QNetworkReply 跟随重定向应可用，真实效果待公子桌面验证）；②CI 修复记录：uchardet 0.0.8 的 pc 文件 Cflags 指向 <include>/uchardet 与其自身安装布局矛盾（Debian 重写过 pc 故 Linux 无感），macOS 用 CMAKE_CXX_FLAGS 补 include 根；vendored ProcessInfo 用 Linux 专属 __uid_t；macOS iconv 不在 libSystem 需显式链接（均记入 VENDOR.md）③下会话：确认 release 资产后 M8 收尾（状态区移除本条），进入 M9 更新健壮化
- 新坑与新决策: ①**冷启动竞态：探测横幅弹出后 500ms 内 launchClient 无条件 `m_banner->hide()` 会杀掉它**——横幅 hide 要判定归属；②herdr status 未运行时输出 `status: not running` 且无 version/compatible 键，exit 0，完全被动；③vendored terminalwidget 的 startShellProgram 只以 isRunning() 为闸，session finished 后可再次 run()；④**开发机装过系统 mudi 包时 DTK 壳命中 /usr/share 旧 qm**——构建树验翻译要 MUDI_UI=generic + staging 跑；⑤xprop 在 Xvfb 读不到 _NET_WM_WINDOW_TYPE，按名筛窗用 getwindowname=="MuDi"；⑥xdotool 合成 Tab 在模态对话框不可靠，用 XTest 坐标点击；⑦`pkill -f` 模式会匹配承载 shell 自杀（加 `[]`）；⑧脚本补丁必须断言真打上；⑨rm .X<n>-lock/socket ≠ 释放显示号——残留 Xvfb 占抽象 namespace socket，必须杀进程；⑩**产品决策：状态带与 herdr 侧边栏同屏冗余，已删**——MuDi 只守窗外（通知+托盘），「项目名 (agent)」（cwd basename，回退 agent→paneId）是托盘/通知统一标识；⑪**GitHub Actions 删推 tag 重触发时，先起的新 run 可能未注册就被 run list 清场脚本误杀**——清理脚本不要在重触发后 30s 内跑；⑫CI 修复三连见断点/下一步②，macOS Intel runner（macos-13）排队极慢（1h+），ARM runner 快得多
- 待公子手动项: GitHub 仓库改名 re2zero/mudi 确认（自更新 API 正式生效）；老 deepin-herdr 包卸载换装 mudi；M7 真实流程验收待下次 herdr 更新；本机 11:34 的 /usr/bin/mudi 遗留进程可关掉换装 0.3.0
