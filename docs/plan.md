...
- 当前里程碑: **M9 更新体系健壮化（已提交 88ef6fb）**——①检查节流：autoCheckUpdates 每 updater 6h 窗口（updater/lastCheckAt/<name>，手动检查不受限），G5 的 60/h 配额消耗从每次启动 2 次降到每天 ≤8 次；②ETag 条件请求：updater/cache/<name>/etag 存 GitHub ETag，If-None-Match 命中 304 时从缓存 Release 回放（304 不计限额），Release 五字段全缓存；③静态 updates.json 源：fetchStaticIndex 先查 gh-pages（UPDATES_JSON_URL=re2zero.github.io/mudi/updates.json，MUDI_UPDATES_URL 可覆盖测试），格式按 installName 分组、assets 按平台键（platformKey 新静态函数）含 url+sha256，缺条目/缺 sha256/404/解析失败全部 fail-closed 回退 API；④安装方式探测：Platform::isDebManaged()（/var/lib/dpkg/info/mudi.list 存在，MUDI_FORCE_PORTABLE=1 可强制反例），deb → BANNER_APP action "View"，便携 → "Update"（G6 策略矛盾闭环）+ 便携自更新完成态 showDone("重启应用生效")/失败 showError。Xvfb 隔离验收全绿：节流跳过与放行双例、静态源 v99.0.0 全链路（本地 http.server 伪造 index → 设置页更新提示+更新按钮）、404 兜底 + ETag 写入 + 304 缓存回放（真网）、deb 正反例日志双证。构建零警告；诊断 qDebug（banner 入队/显示/304 回放）留在文件日志便于日后排查
- 最近提交: 88ef6fb feat: harden the update pipeline (M9)
- 断点/下一步: ①**静态源待公子建 gh-pages**（一次性 10 分钟）：建 re2zero/mudi 的 gh-pages 分支放 updates.json（格式见 updater.cpp fetchStaticIndex 注释，两 updater 条目），后续可让 release CI 自动生成；公子建页前所有检查自动走 API（行为与 v0.3.0 一致，无回归）；②公子真机验收：更新横幅按钮点击流转（Xvfb 下 XTest 点击 banner 按钮无效，疑似 kwin 焦点路由怪异，真机应正常；若真机也无响应则为真 bug 需查）；③之后进入 M10 日常打磨包
