...
#endif
    if (ret.isEmpty()) {
        QDir installDir(QLatin1String(KB_LAYOUT_DIR));
        if (installDir.exists()) {
            ret = installDir.absolutePath();
        }
    }
#ifdef TERMWIDGET_RELOCATABLE_DATADIR
    // VENDOR (see ../VENDOR.md): relocatable fallback for bundled builds
    // (AppImage / Windows portable tree): <appdir>/../share/<libname>/kb-layouts
    if (ret.isEmpty()) {
        QDir portable(d.absoluteFilePath(
            QString::fromLatin1(TERMWIDGET_RELOCATABLE_DATADIR "/kb-layouts")));
        if (portable.exists()) {
            ret = portable.absolutePath();
        }
    }
#endif
